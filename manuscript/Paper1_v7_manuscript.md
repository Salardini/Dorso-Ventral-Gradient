...
Cortical computation emerges from the interplay of neural dynamics across space and time, yet how dynamical properties are spatially organized remains poorly understood. Here we characterize the spatial organization of rotational dynamics—a signature of oscillatory, underdamped neural activity—using resting-state magnetoencephalography (MEG) in 203 healthy adults. We introduce ρ, an index quantifying the rotational component of local dynamics derived from delay-embedded linear models. Across Schaefer-400 parcels, ρ exhibited a striking dorsoventral gradient (r = −0.735 with MNI z-coordinate; p_spin = 0.002), with ventral regions showing stronger rotational dynamics. This gradient was robust to parcellation resolution (Schaefer-100: r = −0.69), replicated across visual and auditory task states, and was independent of both the principal functional connectivity gradient (r = −0.04) and cortical myelin content (r = −0.015). Cross-modal validation in fMRI revealed the same gradient in the MOUS dataset (N = 200; r = −0.246, p_spin < 0.001), with independent replication in the Human Connectome Project (N = 1,093; r = −0.124, p_spin = 0.014). Frequency-specific analysis revealed a spectral trade-off: slow oscillations (delta, theta) showed dorsal predominance while fast oscillations (beta, gamma) showed ventral predominance. Intrinsic timescales (τ) and ρ exhibited strong anticorrelation after removing shared geometry (r = −0.647; p_spin = 0.0002). Computational modeling demonstrated that this τ-ρ trade-off emerges from varying excitation-inhibition balance (r = −0.84, p < 0.001), with functional consequences for temporal pattern generation versus integration. Gene expression analysis using the Allen Human Brain Atlas revealed that somatostatin (SST) interneuron markers—not parvalbumin (PV)—correlate with high ρ (r = +0.124, p = 0.02) and ventral position (r = −0.167, p = 0.002), suggesting that dendritic inhibition by SST interneurons may generate the oscillatory dynamics underlying the ρ gradient.
...
Despite this, τ and ρ showed a striking relationship after accounting for gross anatomy. In raw parcel space, τ and ρ were independent (r = −0.034; p_spin = 0.877). However, after regressing out spatial coordinates (x, y, z) from both maps, τ and ρ residuals showed strong anticorrelation (r = −0.647; p_spin = 0.0002; Figure 4).
...
### Cross-modal validation in fMRI

To test whether the ρ gradient generalizes beyond MEG to hemodynamic signals, we computed ρ from resting-state fMRI in the same MOUS participants (N = 200). Despite fMRI's slower temporal resolution (TR = 2s) and indirect neural coupling, the dorsoventral gradient replicated (r = −0.246; p_spin < 0.001; Figure 5A).

We then tested independent replication in the Human Connectome Project (HCP) using pre-processed Schaefer-400 timeseries (N = 1,096 subjects). The HCP dataset provided an ideal test: different scanner, different site (Washington University), and substantially larger sample. Two preprocessing variants were available: resting-state data with tight bandpass filtering (0.009–0.08 Hz) and task data with broader filtering (0.009–0.25 Hz).

With tight bandpass filtering, the ρ gradient was not detected (r = 0.0003; p_spin = 0.99). Critically, this null result reflected variance compression rather than absence of spatial organization: the tight filter reduced ρ variance 5-fold (range: 0.136–0.157 vs. 0.385–0.487), leaving insufficient signal for spatial correlation. With broader filtering, which preserved higher-frequency BOLD fluctuations, the dorsoventral gradient emerged (r = −0.124; p_spin = 0.014; Figure 5B).

This replication in N = 1,093 independent subjects from a different site, using a different imaging modality, provides strong evidence for the robustness of the ρ gradient. The sensitivity to bandpass filtering also demonstrates an important methodological consideration: preprocessing choices that remove higher-frequency BOLD fluctuations can eliminate the spatial structure of rotational dynamics.
...
### Mechanistic basis of the τ-ρ trade-off: SST interneurons as key players
...
### Cross-modal convergence and preprocessing sensitivity

The replication of the ρ gradient across MEG (r = −0.72) and fMRI (MOUS: r = −0.25; HCP: r = −0.12) provides important cross-modal validation. The attenuated effect size in fMRI likely reflects the hemodynamic response function's temporal smoothing, which blurs the fast dynamics that drive ρ differences. Nevertheless, the consistent gradient direction across modalities suggests a robust underlying organization.

The HCP fMRI analysis revealed critical sensitivity to preprocessing: tight bandpass filtering (0.009–0.08 Hz) compressed ρ variance 5-fold and eliminated the spatial gradient, while broader filtering (0.009–0.25 Hz) preserved it. This demonstrates that higher-frequency BOLD fluctuations (0.08–0.25 Hz) carry spatial information about rotational dynamics that can be inadvertently removed by standard preprocessing pipelines.

### Frequency-specific circuit mechanisms

The spectral trade-off—slow rhythms dorsally, fast rhythms ventrally—suggests distinct circuit mechanisms. Fast oscillations (beta, gamma) are generated by local inhibitory networks, particularly through SST→PV→pyramidal circuits that generate gamma rhythms. These frequencies dominate ventral regions and show strong rotational signatures. Slow oscillations (delta, theta) involve longer-range cortico-cortical connections and cortico-thalamic loops, consistent with their dorsal predominance and weaker local rotational structure.
...
Several limitations should be noted. The AHBA gene expression analysis, while significant, shows modest correlations (|r| ≈ 0.12-0.17), suggesting that interneuron composition explains only a portion of the ρ variance. Other factors—including connectivity, myelination, and laminar organization—likely contribute. The AHBA data derive from post-mortem tissue and may not fully reflect in vivo conditions. Gene expression is an indirect proxy for interneuron density and function; future studies using more direct measures would strengthen these findings.

Our sample (ages 18-45) limits generalizability across the lifespan; cortical inhibitory balance changes with development and aging, potentially shifting the ρ gradient. We lack behavioral correlations; future work should test whether individual differences in ρ gradient strength predict performance on tasks differentially engaging dorsal versus ventral processing.

### Conclusion

We have established a robust dorsoventral gradient of rotational dynamics in human cortex. This gradient represents a novel organizational axis, orthogonal to hierarchical gradients and independent of myeloarchitecture. Cross-modal validation in fMRI and independent replication in over 1,000 HCP subjects establish the robustness of this finding. Computational modeling reveals that the τ-ρ trade-off emerges from excitation-inhibition balance with functional consequences for pattern generation versus integration. Gene expression analysis implicates SST interneurons—not PV interneurons—as the molecular correlates of high rotational dynamics, suggesting that dendritic inhibition shapes the spatial organization of cortical dynamics. These findings open new avenues for understanding how circuit-level properties shape the spatial organization of brain dynamics.
...
MOUS fMRI was acquired with TR = 2s. Preprocessing included motion correction, slice timing correction, bandpass filtering (0.01–0.1 Hz), and nuisance regression (motion parameters, CSF, white matter). Parcel time series were extracted using the same Schaefer-400 atlas.

HCP fMRI used pre-processed Schaefer-400 timeseries from the UCSD-derived dataset (Tipnis et al. 2022; DOI: 10.6075/J0C24WMW). Two variants were analyzed: resting-state (bandpass 0.009–0.08 Hz; N = 1,096) and task (bandpass 0.009–0.25 Hz; N = 1,093). ρ was computed identically to MEG but with embedding parameters adapted for fMRI temporal resolution (embed_dim = 5, embed_delay = 2 TRs).
...
We implemented a rate-based E-I network with 100 excitatory and 25 inhibitory units. Connection weights followed Wilson-Cowan structure: E→E (w=2.0), E→I (w=1.5), I→E (w=1.5×g_I), I→I (w=0.8×g_I), where g_I (inhibitory gain) was varied from 0.3 to 3.5. Dynamics followed τ·dr/dt = -r + f(Wr + I + noise) with softplus activation, τ_E=10ms, τ_I=5ms, and noise σ=0.05. For each g_I, we computed τ (autocorrelation integral) and ρ (delay embedding + VAR) from 2000ms simulations. Pattern richness was measured as effective dimensionality; integration window as impulse response half-life.

### Gene expression analysis

Gene expression data were obtained from the Allen Human Brain Atlas via the abagen toolbox. Expression values for 15,631 genes were averaged across 6 donors and mapped to Schaefer-400 parcels. We focused on interneuron markers: PVALB (PV+ fast-spiking), SST (somatostatin), VIP, GAD1, and GAD2. The PV-SST index was computed as z(PVALB) - z(SST). Correlations with ρ and DV position used Pearson's r with standard parametric p-values.
...
Analysis code is available at https://github.com/Salardini/Dorso-Ventral-Gradient. The MOUS dataset is available through the Donders Repository. HCP MEG and fMRI data are available through the Human Connectome Project. Pre-processed HCP Schaefer timeseries are available from UCSD Library Digital Collections (DOI: 10.6075/J0C24WMW).

---

## FIGURE LEGENDS

### Figure 1. Dorsoventral gradient of rotational dynamics in MEG
**(A)** Cortical surface map of ρ values across 400 Schaefer parcels, showing high ρ (warm colors) in ventral regions and low ρ (cool colors) in dorsal regions. **(B)** Scatter plot of ρ versus MNI z-coordinate (dorsoventral position), revealing strong negative correlation (r = −0.735, p_spin = 0.002). Each point represents one parcel; color indicates network assignment. **(C)** Gradient direction analysis showing the ρ axis is oriented 18° from pure dorsoventral.

### Figure 2. Frequency-specific organization reveals spectral trade-off
Band-specific ρ gradients showing opposite spatial organization for slow versus fast oscillations. Delta and theta bands show dorsal predominance (positive r with z), while alpha, beta, and gamma show ventral predominance (negative r with z). Error bars indicate 95% CI from bootstrap.

### Figure 3. Replication across cognitive states
ρ-DV correlations during rest (r = −0.735), visual task (r = −0.68), and auditory task (r = −0.74), demonstrating stable gradient across cognitive states.

### Figure 4. τ-ρ relationship reveals hidden anticorrelation
**(A)** Raw τ versus ρ across parcels shows no relationship (r = −0.03). **(B)** After regressing out spatial coordinates (x, y, z), τ and ρ residuals show strong anticorrelation (r = −0.65, p_spin = 0.0002), suggesting a local circuit trade-off.

### Figure 5. Cross-modal validation in fMRI
**(A)** MOUS fMRI replicates the ρ-DV gradient (N = 200; r = −0.246, p_spin < 0.001). **(B)** HCP fMRI with broad bandpass filtering independently replicates the gradient (N = 1,093; r = −0.124, p_spin = 0.014). **(C)** Comparison of effect sizes across datasets and modalities.

### Figure 6. Computational modeling and gene expression reveal mechanistic basis
**(A)** Rate-based E-I network model: varying inhibitory gain (g_I) produces opposing effects on τ (blue) and ρ (red). **(B)** The τ-ρ trade-off emerges naturally from E-I balance (r = −0.84, p < 0.001); color indicates g_I value. **(C)** AHBA gene expression correlations with ρ: SST shows significant positive correlation (r = +0.12*), while PVALB is non-significant; PV-SST ratio correlates negatively with ρ (r = −0.12*). **(D)** Revised mechanistic model: SST interneurons provide dendritic inhibition creating oscillatory dynamics (high ρ) in ventral cortex, while high PV/SST ratio provides somatic stabilization creating stable dynamics (low ρ) in dorsal cortex.
...
**Panel E: Computational model results**

| Metric | r | p | Interpretation |
|--------|---|---|----------------|
| τ-ρ trade-off | −0.84 | 0.0006 | Emerges from E-I balance |
| ρ → Pattern richness | +0.87 | 0.0003 | High ρ = richer dynamics |
| τ → Integration window | +0.86 | 0.0003 | High τ = longer memory |

**Panel F: AHBA gene expression correlations**

| Gene | r vs ρ | p | r vs DV | p | Interpretation |
|------|--------|---|---------|---|----------------|
| PVALB | −0.085 | 0.11 | +0.078 | 0.15 | Not significant |
| SST | +0.124 | 0.02 | −0.167 | 0.002 | Ventral, high ρ |
| VIP | −0.010 | 0.86 | +0.041 | 0.45 | Not significant |
| GAD1 | −0.025 | 0.64 | +0.049 | 0.37 | Not significant |
| PV-SST | −0.121 | 0.02 | +0.142 | 0.008 | Dorsal, low ρ |

**Panel G: Cross-modal and independent replication**

| Dataset | Modality | N | Filter (Hz) | r (ρ-DV) | p_spin |
|---------|----------|---|-------------|----------|--------|
| MOUS | MEG | 203 | 1–40 | −0.735 | 0.002 |
| MOUS | fMRI | 200 | 0.01–0.1 | −0.246 | <0.001 |
| HCP | fMRI (REST) | 1,096 | 0.009–0.08 | +0.0003 | 0.99 |
| HCP | fMRI (TASK) | 1,093 | 0.009–0.25 | −0.124 | 0.014 |
...
11. Urban-Ciecko J, Bharat AA (2016) Somatostatin-expressing neurons in cortical networks. Nat Rev Neurosci 17:401-409.

12. Cardin JA (2018) Inhibitory interneurons regulate temporal precision and correlations in cortical circuits. Trends Neurosci 41:689-700.

13. Silberberg G, Markram H (2007) Disynaptic inhibition between neocortical pyramidal cells mediated by Martinotti cells. Neuron 53:735-746.

14. Hawrylycz MJ, et al. (2012) An anatomically comprehensive atlas of the adult human brain transcriptome. Nature 489:391-399.

15. Arnatkevičiūtė A, et al. (2019) A practical guide to linking brain-wide gene expression and neuroimaging data. NeuroImage 189:353-367.

16. Tipnis U, et al. (2022) Derived Products from HCP-YA fMRI. UC San Diego Library Digital Collections. DOI: 10.6075/J0C24WMW.

17. Murray JD, et al. (2014) A hierarchy of intrinsic timescales across primate cortex. Nat Neurosci 17:1661-1663.

18. Wilson HR, Cowan JD (1972) Excitatory and inhibitory interactions in localized populations of model neurons. Biophys J 12:1-24.
