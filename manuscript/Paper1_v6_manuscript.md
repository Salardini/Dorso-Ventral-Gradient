...
To test whether the ρ gradient generalizes beyond MEG to hemodynamic signals, we computed ρ from resting-state fMRI in the same MOUS participants (N = 200). Despite fMRI's slower temporal resolution (TR = 2s) and indirect neural coupling, the dorsoventral gradient replicated (r = −0.246; p_spin < 0.001; Figure 3A).
...
### Partial replication in HCP MEG

In an independent MEG dataset (Human Connectome Project MEG, N = 89), we tested replication using pre-processed power envelope time series parcellated to 107 regions. The gamma-low band showed a correlation consistent with MOUS findings (r = −0.40), though this did not survive spin permutation (p_spin = 0.12).
...
Third, the weaker HCP MEG replication reflects methodological differences rather than non-reproducibility. HCP's power envelope time series fundamentally differ from raw broadband signals—envelopes capture amplitude fluctuations but lose phase information critical to rotational dynamics. In contrast, the HCP fMRI replication with appropriate preprocessing (broader bandpass) successfully detected the gradient in over 1,000 subjects.
...
For the Human Connectome Project MEG (N=89), we used pre-processed power envelope time series (band-filtered, Hilbert amplitude) parcellated to Yeo-17 split atlas (~107 regions). ρ was computed identically to MOUS.
...
**Panel F: Cross-modal and independent replication**

| Dataset | Modality | N | Filter (Hz) | ρ range | r (ρ-DV) | p_spin |
|---------|----------|---|-------------|---------|----------|--------|
| MOUS | MEG | 203 | 1–40 | 0.58–0.61 | −0.735 | 0.002 |
| MOUS | fMRI | 200 | 0.01–0.1 | 0.55–0.60 | −0.246 | <0.001 |
| HCP | fMRI (REST) | 1,096 | 0.009–0.08 | 0.136–0.157 | +0.0003 | 0.99 |
| HCP | fMRI (TASK) | 1,093 | 0.009–0.25 | 0.385–0.487 | −0.124 | 0.014 |
| HCP | MEG | 89 | gamma envelope | — | −0.40 | 0.12 |
...
11. Tipnis U, et al. (2022) Derived Products from HCP-YA fMRI. UC San Diego Library Digital Collections. DOI: 10.6075/J0C24WMW.
