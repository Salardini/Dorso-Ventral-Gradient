...
- Individual subject MEG timeseries (parcel_ts.npy, ~204 subjects) and per-subject metric CSVs are not included due to size. To regenerate: (1) download the MOUS dataset from OpenNeuro (https://openneuro.org/datasets/ds004998); (2) update the `bids_root` path in `config/config.yaml`; (3) run the pipeline scripts in order: `00_extract_tarballs.py` → `01_reconall.sh` → `02_make_bem.py` → (manual coregistration) → `04_extract_parcels_and_metrics.py` → `05_group_stats.py`. See `scripts/pipeline/smoke_test_one_subject.sh` for a single-subject test run.
