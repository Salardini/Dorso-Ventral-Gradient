#!/usr/bin/env python3
"""
AHBA Analysis - Fixed for pandas 2.x compatibility
===================================================

The issue is abagen uses deprecated pandas syntax.
This script downgrades the problematic call.

Run: python ahba_fixed.py
"""
...
def main():
    ...
    print("=" * 60)
    print("AHBA Gene Expression Analysis (pandas 2.x fix)")
    print("=" * 60)
    ...
    # PATCH: Fix pandas 2.x incompatibility in abagen
    print("\n[3] Applying pandas 2.x compatibility patch...")
    
    # Patch the problematic function in abagen
    import abagen.probes_ as probes_mod
    import functools
    
    # Store original
    _original_collapse = probes_mod.collapse_probes
    
    @functools.wraps(_original_collapse)
    def _patched_collapse(*args, **kwargs):
        # Monkey-patch DataFrame.set_axis temporarily
        _orig_set_axis = pd.DataFrame.set_axis
        
        def _fixed_set_axis(self, labels, *, axis=0, copy=None, **kw):
            # Ignore 'inplace' argument
            kw.pop('inplace', None)
            return _orig_set_axis(self, labels, axis=axis, copy=copy)
        
        pd.DataFrame.set_axis = _fixed_set_axis
        try:
            return _original_collapse(*args, **kwargs)
        finally:
            pd.DataFrame.set_axis = _orig_set_axis
    
    probes_mod.collapse_probes = _patched_collapse
    
    # Now get expression data
